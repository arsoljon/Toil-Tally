...
class ClockedInFrame(tk.Frame):
    def __init__(self, parent, ui_controller, clock_in_controller,state):
        super().__init__(parent)
        state.running = True 
        
        self.clock_in_controller = clock_in_controller
        self.ui_controller = ui_controller
        self.buttonLabels = ui_controller.clockedIn_buttons
        #text
        ...
        time = clock_in_controller.get_session_length()
        ...
        #buttons
        self.button1 = tk.Button(self.buttonFrame_end_pause, text=self.buttonLabels[0], command= lambda: self.on_click_end(ui_controller, state))
        self.button1.grid(row=0, column=0, sticky="w", padx = 10)
        self.button2 = tk.Button(self.buttonFrame_end_pause, text=self.buttonLabels[1], command= lambda: self.on_click_pause(ui_controller, state))
        self.button2.grid(row=0, column=1, sticky="e", padx = 200)
        self.button3 = tk.Button(self.buttonFrame_update,text=self.buttonLabels[2], command= lambda: self.on_click_update(state))
        self.button3.grid(row=0, column=0)
    
    def on_click_update(self, state):
        self.clock_in_controller.update_session()

    def on_click_end(self, ui_controller, state):
        #end
        self.clock_in_controller.add_session_to_job()
        ui_controller.show_frame(ui_controller.clockedIn_pages[0], state)

    def on_click_pause(self, ui_controller, state):
        #pause
        ui_controller.show_frame(ui_controller.clockedIn_pages[1], state)
    ...
    def update_display(self, state):
        hh, mm, ss = self.clock_in_controller.update_session_display()
